...
int udp_driver_write(unsigned char * data, int size)
{
	...
	#ifdef DEBUG
		printf("[Debug] L%d %s : (%d bytes to %s:%d) %s\n",__LINE__,__FUNCTION__,cnt_bytes,inet_ntoa(remote_sockaddr.sin_addr),ntohs(remote_sockaddr.sin_port),data);	
	#endif
	return 0;
}
...
int udp_driver_read(unsigned char * data, int size)
{
	...
	#ifdef DEBUG
		printf("[Debug] L%d %s : (%d bytes from %s:%d) %s\n",__LINE__,__FUNCTION__,cnt_bytes,inet_ntoa(sender_sockaddr.sin_addr),ntohs(sender_sockaddr.sin_port),data);	
	#endif
	return 0;
}
